...
# -*- coding: utf-8 -*-
"""
Security-aware probe and hue-mapping search for 2 legal positions.

The script searches every 2-position combination in data/15pro/yellow_shuffled
and writes successful probe sets to yellow_shuffled_security_aware_safe_probes_2.csv.
Existing successful combinations in that CSV are skipped on restart.

Security target: legal devices decode with raw BER = 0, and min_illegal_ber > 0.3,
where min_illegal_ber is the secure BER min(BER, 1-BER) over all illegal positions
and legal routes.
"""
...
OUTPUT_RESULTS_FILENAME = "yellow_shuffled_security_aware_safe_probes_2.csv"
TARGET_LEGAL_BER = 0.0
MIN_ILLEGAL_BER = 0.3
MAX_ILLEGAL_BER = 0.7
...
EVAL_BIT_BLOCK_COUNT = 1000
EVAL_BIT_BLOCK_SEED = 20260509
ILLEGAL_AWARE_CANDIDATES_PER_COUNT = 24
ILLEGAL_AWARE_POOL_PER_SYMBOL = 12


def load_test_module() -> types.ModuleType:
    module_name = "test_3_simple_runtime_security_search_2"
...
def build_position_models(
    ...
    matrices = test.load_selected_rows(csv_files, probes_array)
    return {
        int(position): test.extract_fingerprint(probes_array, matrix, force_positive_first=True)
...
def generate_random_bit_blocks(num_positions: int, count: int, seed: int) -> list[np.ndarray]:
    rng = np.random.default_rng(seed)
    blocks = rng.choice([-1, 1], size=(count, num_positions))
    return [np.asarray(bits_pm, dtype=int) for bits_pm in blocks]


def evaluation_seed(legal_positions: Sequence[int], probes: Sequence[float]) -> int:
    position_part = sum((idx + 1) * int(position) * 1009 for idx, position in enumerate(legal_positions))
    probe_part = sum((idx + 1) * int(round(float(probe))) for idx, probe in enumerate(np.sort(probes)))
    return EVAL_BIT_BLOCK_SEED + position_part + probe_part
...
def legal_decode_is_exact(legal_position_bers: Sequence[float]) -> bool:
    return all(float(ber) == TARGET_LEGAL_BER for ber in legal_position_bers)


def security_is_satisfied(candidate: dict) -> bool:
    return (
        legal_decode_is_exact(candidate["legal_position_bers"])
...
def find_security_aware_hue_mapping(
    ...
    bit_blocks_pm = generate_random_bit_blocks(
        len(legal_positions),
        EVAL_BIT_BLOCK_COUNT,
        evaluation_seed(legal_positions, probes),
    )
    ...
    for hue_mapping in mappings:
        ...
        if not legal_decode_is_exact(legal_position_bers):
            candidate = {
                "legal_ber": legal_ber,
        ...
        candidate["security_satisfied"] = security_is_satisfied(candidate)
...
def candidate_rank_key(candidate: dict) -> tuple[float, float, float, float, float]:
    ...
    legal_ok = 1.0 if legal_decode_is_exact(candidate["legal_position_bers"]) else 0.0
...
def evaluate_candidate(project_root: str, legal_positions: Sequence[int], probes: Sequence[float]) -> dict:
    ...
    result["security_satisfied"] = security_is_satisfied(result)
    return result
...
def append_probe_with_spacing(selected: list[float], probe: float, min_interval: int) -> bool:
    probe = float(probe)
    if probe in selected:
        return False
    if any(abs(probe - existing) < min_interval for existing in selected):
        return False
    selected.append(probe)
    return True


def generate_illegal_aware_probe_sets(
    project_root: str,
    legal_positions: Sequence[int],
    all_probes: Sequence[float],
    probe_count: int,
    ...
    models = build_position_models(project_root, all_positions, all_probes)
    legal_models = test.align_model_directions([models[int(position)] for position in legal_positions])
    illegal_models = [models[position] for position in all_positions if position not in legal_positions]
    probes_array = np.asarray(all_probes, dtype=float)

    illegal_abs_z = np.vstack([np.abs(model.z) for model in illegal_models]) if illegal_models else np.zeros((1, len(probes_array)))
    illegal_mean_strength = np.mean(illegal_abs_z, axis=0)
    illegal_max_strength = np.max(illegal_abs_z, axis=0)
    legal_abs_z = np.vstack([np.abs(model.z) for model in legal_models])
    legal_strength = np.min(legal_abs_z, axis=0) + 0.25 * np.mean(legal_abs_z, axis=0)
    probe_scores = legal_strength / (1.0 + illegal_mean_strength + 0.5 * illegal_max_strength)

    symbol_pools: dict[tuple[int, ...], list[float]] = {}
    for symbol in itertools.product([-1, 1], repeat=len(legal_models)):
        scored: list[tuple[float, float]] = []
        for idx, probe in enumerate(probes_array):
            signed_margins = [
                int(sign) * float(model.z[idx])
                for sign, model in zip(symbol, legal_models)
            ]
            matched = sum(1 for margin in signed_margins if margin > 0)
            legal_margin = sum(max(0.0, margin) for margin in signed_margins)
            mismatch_penalty = sum(abs(margin) for margin in signed_margins if margin <= 0)
            score = matched * 1000.0 + legal_margin - 2.0 * mismatch_penalty - illegal_mean_strength[idx]
            scored.append((float(score), float(probe)))
        scored.sort(reverse=True)
        symbol_pools[symbol] = [probe for _, probe in scored[:ILLEGAL_AWARE_POOL_PER_SYMBOL]]

    global_ranked = [float(probes_array[idx]) for idx in np.argsort(probe_scores)[::-1]]
    candidate_sets: list[np.ndarray] = []
    seen: set[tuple[float, ...]] = set()

    for offset in range(max_sets * 4):
        selected: list[float] = []
        symbols = list(symbol_pools.keys())
        symbols = symbols[offset % len(symbols):] + symbols[:offset % len(symbols)]
        for symbol_idx, symbol in enumerate(symbols):
            pool = symbol_pools[symbol]
            for probe in pool[offset % max(1, min(len(pool), ILLEGAL_AWARE_POOL_PER_SYMBOL)):] + pool[:offset % max(1, min(len(pool), ILLEGAL_AWARE_POOL_PER_SYMBOL))]:
                if append_probe_with_spacing(selected, probe, min_interval):
                    break
            if len(selected) >= probe_count:
                break

        ranked = global_ranked[offset % len(global_ranked):] + global_ranked[:offset % len(global_ranked)]
        for probe in ranked:
            append_probe_with_spacing(selected, probe, min_interval)
            if len(selected) >= probe_count:
                break

        if len(selected) != probe_count:
            continue
        probes = tuple(float(v) for v in np.sort(np.asarray(selected, dtype=float)))
        if probes in seen:
            continue
        seen.add(probes)
        candidate_sets.append(np.asarray(probes, dtype=float))
...
def search_security_aware_probes(
    ...
    for epoch in range(SEARCH_EPOCHS):
        ...
        for probe_count in candidate_counts:
            ...
            probe_sets = generate_illegal_aware_probe_sets(
                project_root=project_root,
                legal_positions=legal_positions,
                all_probes=all_probes,
                probe_count=probe_count,
                max_sets=ILLEGAL_AWARE_CANDIDATES_PER_COUNT,
            )
...
def run_search() -> str:
    ...
    results_file = os.path.join(project_root, "test-2", OUTPUT_RESULTS_FILENAME)
