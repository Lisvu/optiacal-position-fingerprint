...
DATASET_NAME = "15pro_yellow_shuffled"
DATASET_DIR = os.path.join(PROJECT_ROOT, "data", "15pro", "yellow_shuffled")
...
def choose_best_candidate(
    ...
    for candidate_id, mapping in enumerate(mappings, start=1):
        print(f"Evaluating mapping candidate {candidate_id}/{len(mappings)}...", flush=True)
        ...
        print(
            "  "
            f"authorized_max_ber={candidate.authorized_max_ber:.8f}, "
            f"illegal_min_leakage_ber={candidate.min_route_min_ber:.8f}, "
            f"illegal_avg_leakage_ber={candidate.average_route_min_ber:.8f}",
            flush=True,
        )
