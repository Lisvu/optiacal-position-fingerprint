...
class FingerprintModel:
    probes: Array              # shape: (P,)
    Y: Array                   # shape: (P, D)
    trend: Array               # shape: (P, D)
    residual: Array            # shape: (P, D)
    w: Array                   # shape: (D,)
    z: Array                   # shape: (P,)
    code: Array                # shape: (P,)
...
@dataclass
class DecodeResult:
    Y_obs: Array               # shape: (L, D)
    mean_vec: Array            # shape: (D,)
    Y_centered: Array          # shape: (L, D)
    u: Array                   # shape: (L,)
    gamma: float
    bit_hat_pm: int            # +1 / -1
    bit_hat_bin: int           # 1 / 0
...
def extract_fingerprint(x: Array, Y: Array, force_positive_first: bool = True) -> FingerprintModel:
    ...
    1) 逐列线性去趋势
    2) 对残差矩阵做 SVD
    3) 取第一右奇异向量作为本地读出方向 w
    4) 投影 z = R w
    5) 用 sign(z) 生成地址码 c

    注：SVD 符号本身不唯一。默认将 z[0] 调整为非负，便于不同运行之间结果一致。
    """
